...
public class ContactBook {
	...
	private int samePhone() {
		int result = 0;
		int name = 0;
		while (name < counter - 1 && result == 0) {
			int phone = name + 1;
			while (phone < counter && result == 0) {
				if (contacts[name].getPhone() == contacts[phone].getPhone())
					result++;
				phone++;
			}
			name++;
